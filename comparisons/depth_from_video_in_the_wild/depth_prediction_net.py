...
def encoder_resnet(target_image, weight_reg, is_training, normalizer_fn=None):
  ...
  """
    ...
    # conv3_x
    x = s_residual_block_first( econv2, is_training, encoder_filters[2], stride, name='conv3_1', normalizer_fn=normalizer_fn)
    econv3 = s_residual_block(x, is_training, name='conv3_2')

    # conv4_x
    x = s_residual_block_first( econv3, is_training, encoder_filters[3], stride, name='conv4_1', normalizer_fn=normalizer_fn)
    econv4 = s_residual_block(x, is_training, name='conv4_2')

    # conv5_x
    x = s_residual_block_first(econv4, is_training, encoder_filters[4], stride, name='conv5_1', normalizer_fn=normalizer_fn)
    econv5 = s_residual_block( x, is_training, name='conv5_2', normalizer_fn=normalizer_fn)
...
def depth_prediction_resnet18unet(images, is_training, decoder_weight_reg=0.0, normalizer_fn=None, reflect_padding=True):
  ...
  """
    ...
    bottleneck, skip_connections = encoder_resnet(images, weight_reg=None, is_training=is_training,
...
def s_residual_block_first(x,  is_training,  out_channel, strides, name='unit', normalizer_fn=None):
...
def s_residual_block(x, is_training, input_q=None, output_q=None, name='unit', normalizer_fn=None):
    """Helper function for defining ResNet architecture."""
    normalizer_fn = normalizer_fn or _bn
    ...
    with tf.variable_scope(name):
        ...
        # Residual
        x = s_conv(x, 3, num_channel, 1, input_q=input_q, output_q=output_q, name='conv_1')
        x = normalizer_fn(x, is_train=is_training, name='bn_1')
        x = s_relu(x, name='relu_1')
        x = s_conv(x, 3, num_channel, 1, input_q=output_q, output_q=output_q, name='conv_2')
        x = normalizer_fn(x, is_train=is_training, name='bn_2')
        # Merge
        x = x + shortcut
        x = s_relu(x, name='relu_2')
