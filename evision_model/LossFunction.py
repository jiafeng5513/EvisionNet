...
正确认识这个式子的含义是理解EvisionNet训练理论的基础.
    ...
    答3: 可以通过插值,把P1落回到整数网格坐标,具体来讲:
        ...
        d.DFVNet 实际上采用了一种截然相反的策略,他不是对重投影深度图和重投影RGB图像做插值采样,而是对原图做采样,将原本处于整数坐标处
          的原图像素或者深度值转换到与之对比的浮点数坐标下.其动机可能是为了利用tf.contrib.resampler.resampler,
          该函数的作用就是实现向用户指定的坐标点进行采样
