...
{
	// Performing our own mapping
	CvMat *S = cvCreateMat(2,2,CV_32FC1);
	CvMat *invS = cvCreateMat(2,2,CV_32FC1);
	float *S_data = S->data.fl;	
	for (int i = 0;i < n;i++)
	{
		S_data[0] = pKeys[i].a;
		S_data[1] = pKeys[i].b;
		...
		cvInvert(S, invS);
		// from VLFeat code
		float a = (float)sqrt(invS->data.fl[0]);
		float b = invS->data.fl[1] / MAX(a, 1e-18);
		float c = (float)sqrt(MAX(invS->data.fl[3]-(b*b),0.));
		// A = [a, 0; b, c]

		// cos and sin
		float rad = pKeys[i].angle / 180.0 * CV_PI;
		float cos_val = cos(rad);
		float sin_val = sin(rad);		
		// R = [cos, -sin; sin, cos]

		// Affine = A*R
		pKeys[i].trans[0] = a*cos_val;
		pKeys[i].trans[1] = -a*sin_val;
		pKeys[i].trans[2] = b*cos_val + c*sin_val;
		pKeys[i].trans[3] = -b*sin_val + c*cos_val;
	}
	cvReleaseMat(&S);
	cvReleaseMat(&invS);
