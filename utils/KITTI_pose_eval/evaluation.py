...
class kittiOdomEval():
    def __init__(self, config):
        ...
        self.eval_seqs = []
        self.save_png = config.savepng
        self.save_pdf = config.savepdf
        self.show_gui = config.show
    ...
    def plotPath_2D_3(self, seq, poses_gt, poses_result, plot_path_dir):
        ...
        plt.legend(loc="upper right", prop={'size': fontsize_[0]})
        ...
        plt.plot(start_point[0], start_point[1], style_O, label='起点')
        plt.legend(loc="upper right", prop={'size': fontsize_[1]})
        plt.xlabel('x (m)', fontsize=fontsize_[1])
        plt.ylabel('y (m)', fontsize=fontsize_[1])
        xlim = ax.get_xlim()
        ylim = ax.get_ylim()
        xmean = np.mean(xlim)
        ymean = np.mean(ylim)
        ax.set_xlim([xmean - plot_radius, xmean + plot_radius])
        ax.set_ylim([ymean - plot_radius, ymean + plot_radius])
        plt.legend(loc='lower left', fontsize=fontsize_[1])  # 标签位置
        ...
        plt.plot(start_point[0], start_point[1], style_O, label='起点')
        plt.legend(loc="upper right", prop={'size': fontsize_[2]})
        plt.xlabel('y (m)', fontsize=fontsize_[2])
        plt.ylabel('z (m)', fontsize=fontsize_[2])
        xlim = ax.get_xlim()
        ylim = ax.get_ylim()
        xmean = np.mean(xlim)
        ymean = np.mean(ylim)
        ax.set_xlim([xmean - plot_radius, xmean + plot_radius])
        ax.set_ylim([ymean - plot_radius, ymean + plot_radius])
        plt.legend(loc='lower left', fontsize=fontsize_[2])  # 标签位置
    ...
    def plotPath_3D(self, seq, poses_gt, poses_result, plot_path_dir):
        ...
        poses_dict = {}
        poses_dict["EvisionNet"] = poses_result
        if poses_gt:
        ...
        for key, _ in poses_dict.items():
            ...
            style = style_pred if key == 'EvisionNet' else style_gt
            plt.plot(plane_point[:, 0], plane_point[:, 1], plane_point[:, 2], style, label=key)
        plt.plot(start_point[0], start_point[1], start_point[2], style_O, label='起点')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--notshow', dest='show', action='store_false', default=True, help='DO NOT Show imgs with GUI')
    parser.add_argument('--pdf', dest='savepdf', action='store_true', default=False, help='save imgs into pdf')
    parser.add_argument('--png', dest='savepng', action='store_true', default=False, help='save imgs into pdf')
